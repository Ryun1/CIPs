...
All 9 fields are **required**, must appear in order, and no extra fields are allowed.
...
For the **Proposed Solutions** and **Discussions** fields, when an entry label matches the `CIP-NNNN` pattern, extra validation applies:
...
The following sections are allowed with exact capitalization (can appear between required sections):
